...
class CodeBattles(Generic[GameStateType, GameContextType, APIType]):
    ...
    @property
    def time(self):
        return self.step / self.get_steps_per_second()
            ...
            if api_code != "":
                ...
                try:
                    ...
                    for i in string_file_indices:
                        output += (
                            lines[i].strip().replace('File "<string>", l', "L") + "\n"
                        )
    ...
    def _step(self):
        ...
        if not self.background:
            ...
            document.getElementById(
                "noui-progress"
            ).textContent = f"Simulating time {self.state.time}s..."
            if self.over:
    ...
    def _should_play(self):
        ...
        if self.time == self._get_breakpoint():
            return False
